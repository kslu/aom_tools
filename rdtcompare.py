...
def get_rdt(filename):
  ...
  seq_info = [
      float(csp_seq_info[0]),
      float(csp_seq_info[1]),
      float(csp_seq_info[2])
  ]
...
def main():
  ...
  for r in range(num_rdt):
    bps1_all[r], psnr1_all[r], enctime1_all[r], bpf1_all[r], seq_info_all[
        r] = get_rdt(files1[r])
  for r in range(num_rdt):
    bps2_all[r], psnr2_all[r], enctime2_all[r], bpf2_all[r], _ = get_rdt(
        files2[r])

  # compute global PSNRs (average over Y, U, V)
  mse1_all = 255**2 / (10**(psnr1_all / 10))
  mse2_all = 255**2 / (10**(psnr2_all / 10))
  gmse1_all = (4 * mse1_all[:, :, 0] + mse1_all[:, :, 1] +
               mse1_all[:, :, 2]) / 6
  gmse2_all = (4 * mse2_all[:, :, 0] + mse2_all[:, :, 1] +
               mse2_all[:, :, 2]) / 6
